...
{
	m_p_current_scene = std::make_unique<SceneTitle>(m_game_data, m_p_story_manager);
	m_scene_state = SceneState::Title;

	m_p_sound_manager = std::make_shared<SoundManager>();
	m_p_story_manager = std::make_shared<StoryManager>(m_game_data);
}
		...
		if (m_wait_count >= WAIT_TIME)
		{
			...
			case SceneState::Title:
			{
				...
				case SceneBase::SceneResult::NewGame:
				{
					...
					std::unique_ptr<SceneMain> p_scene_main = std::make_unique<SceneMain>(m_game_data, m_p_story_manager);
			...
			case SceneState::Play:
			{
				...
				{
					std::unique_ptr<SceneTitle> p_scene_title = std::make_unique<SceneTitle>(m_game_data, m_p_story_manager);
				...
				{
					std::unique_ptr<SceneClear> p_scene_clear = std::make_unique<SceneClear>(m_game_data, m_p_story_manager);
				...
				{
					std::unique_ptr<SceneGameover> p_scene_gameover = std::make_unique<SceneGameover>(m_game_data, m_p_story_manager);
